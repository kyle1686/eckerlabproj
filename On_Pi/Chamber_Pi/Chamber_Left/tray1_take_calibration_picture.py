...
def get_file_name():
    save_path = paths.cl_t1_calibimage_save_path
...
def copy_to_drive():
    try:
        subprocess.run(
            ["rclone", "copy", paths.cl_t1_calibimage_save_path, paths.cl_gdrive_calibration_path],
            check=True
        )
        print(f"Successfully copied {paths.cl_t1_calibimage_save_path} → {paths.cl_gdrive_calibration_path}")
