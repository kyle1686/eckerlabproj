...
def get_file_name():
    save_path = paths.cl_t2_calibimage_save_path
    os.makedirs(save_path, exist_ok=True) # Ensure directory exists
    filename = os.path.join(save_path, f"Tray2_calibration_image_{get_current_time()}.jpg")
    return filename
...
def copy_to_drive():
    try:
        subprocess.run(
            ["rclone", "copy", paths.cl_t2_calibimage_save_path, paths.cl_gdrive_calibration_path],
            check=True
        )
        print(f"Successfully copied {paths.cl_t2_calibimage_save_path} → {paths.cl_gdrive_calibration_path}")
