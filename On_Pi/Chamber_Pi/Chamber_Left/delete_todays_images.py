...
def recopy_to_gdrive():
    try:
        subprocess.run(
            ["rclone", "copy", paths.cl_image_save_path, paths.cl_gdrive_remote_path],
            check=True
        )
        print(f"Successfully copied {paths.cl_image_save_path} → {paths.cl_gdrive_remote_path}")
    except subprocess.CalledProcessError as e:
        print(f"Upload failed: {e}")
    except FileNotFoundError:
        print("rclone not found — make sure it's installed and in PATH.")
    
def delete_temp_images(directory = paths.cl_image_save_path):
    deleted = 0
