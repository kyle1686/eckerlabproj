...
cm_repo_root = '/home/user2/Salk_Project_Folder/eckerlabproj'
...
def get_file_name():
    save_path = paths.cm_checkerboard_calibration_onpi
    os.makedirs(save_path, exist_ok=True) # Ensure directory exists
    filename = os.path.join(save_path, f"ChamberMiddle_checkerboard_{get_current_time()}.jpg")
    return filename
...
def copy_to_drive():
    try:
        subprocess.run(
            ["rclone", "copy", paths.cm_checkerboard_calibration_onpi, paths.cm_gdrive_checkerboard_images],
            check=True
        )
        print(f"Successfully copied {paths.cm_checkerboard_calibration_onpi} → {paths.cm_gdrive_checkerboard_images}")
