...
def get_file_name():
    save_path = paths.cm_t5_calibimage_save_path
    os.makedirs(save_path, exist_ok=True) # Ensure directory exists
    filename = os.path.join(save_path, f"Tray5_calibration_image_{get_current_time()}.jpg")
    return filename
...
def copy_to_drive():
    try:
        subprocess.run(
            ["rclone", "copy", paths.cm_t5_calibimage_save_path, paths.cm_gdrive_calibration_path],
            check=True
        )
        print(f"Successfully copied {paths.cm_t5_calibimage_save_path} → {paths.cm_gdrive_calibration_path}")
